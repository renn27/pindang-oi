...
<x-common.page-breadcrumb pageTitle="{{$title}}" />
    ...
    @php
        $years = $jenis_kegiatans->map(function($jk) {
            return $jk->created_at->format('Y');
        })->unique()->sort()->values();
        $currentYear = date('Y');
            ...
            <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-900 dark:divide-gray-700">
                <tr id="noDataRow" class="hidden">
                    <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                        Tidak ada data jenis kegiatan untuk tahun yang dipilih.
                    </td>
                </tr>
                @foreach ($jenis_kegiatans as $index => $jenisKegiatan)
                    <tr class="jk-row hover:bg-gray-50 dark:hover:bg-gray-800" data-tahun="{{ $jenisKegiatan->created_at->format('Y') }}">
                        <td class="jk-index px-4 py-3 text-sm font-medium text-gray-900 text-center dark:text-gray-300">
                            {{ $index + 1 }}
                        </td>
    ...
    @push('scripts')
    <script>
        function filterJkByTahun(selectedYear) {
            ...
            rows.forEach(row => {
                if (!row.classList.contains('hidden')) {
                    const indexCell = row.querySelector('.jk-index');
                    if (indexCell) {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (selectFilter) {
                filterJkByTahun(selectFilter.value);
                selectFilter.addEventListener('change', function() {
                    filterJkByTahun(this.value);
                });
            }
        });
