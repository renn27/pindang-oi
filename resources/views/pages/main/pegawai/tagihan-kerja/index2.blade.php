...
@section('content')
    ...
    <!-- Smart Modal untuk Kegiatan -->
    <x-ui.smart-modal id="modal-kegiatan-rk-ketua" class="max-w-xl">
        <!-- Modal content akan diisi oleh Alpine.js -->
    </x-ui.smart-modal>

    <!-- Template untuk modal content (akan di-copy ke smart-modal) -->
    <template id="modal-kegiatan-template">
        <div
            ...
            <div class="flex-1 overflow-y-auto px-6 py-5 custom-scrollbar">
                <form x-bind:action="mode === 'edit' ? actionUrl : '{{ route('kegiatan.store', $bidang->slug) }}'"
                    method="POST"
                    ...
                    <div x-data="pegawaiAutocomplete()">
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                            Nama Ketua
                        </label>
                        <!-- Input search -->
                        ...
                        <!-- Dropdown -->
                        <div x-show="open && filteredPegawais.length > 0"
                            x-transition
                            class="absolute z-50 mt-1 w-full max-w-[calc(700px-3rem)] rounded-lg border border-gray-300 bg-white dark:bg-gray-800 max-h-60 overflow-y-auto shadow-lg">
                            <template x-for="(pegawai, index) in filteredPegawais" :key="pegawai.id_pegawai">
                                <div @click="selectPegawai(pegawai)"
                                    :class="{'bg-blue-100 dark:bg-blue-900': highlightedIndex===index, 'hover:bg-gray-100 dark:hover:bg-gray-700': highlightedIndex!==index}"
                                    class="cursor-pointer px-4 py-2 text-sm text-gray-800 dark:text-gray-200"
                    ...
                    <div>
                        ...
                        <input type="text"
                            x-model="formData.tahun_kegiatan"
                            name="tahun_kegiatan"
                    ...
                    <div>
                        ...
                        <select
                            ...
                            class="dark:bg-dark-900 h-11 w-full appearance-none rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800"
                            required>
                            <option value="">-- Pilih IKI JPT --</option>
                            <template x-for="iki in ikiOptions" :key="iki.id">
                                <option :value="iki.id" x-text="iki.nama_indikator_jpt"></option>
                            </template>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text"
                            x-model="formData.nama_rk_kegiatan"
                            name="nama_rk_kegiatan"
                    ...
                    <div class="shrink-0 border-t border-gray-200 px-6 py-3 dark:border-gray-800">
                        <div class="flex justify-end gap-3">
                            <button type="button"
                                @click="$dispatch('close-smart-modal', { modalId: 'modal-kegiatan-rk-ketua' })"
                ...
                <div class="bg-white dark:bg-white/[0.03] px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button
                                @click="openEditModal(@js($kegiatan))"
                                class="flex items-center gap-2 rounded-lg border border-blue-300 bg-white px-4 py-2 text-sm font-medium text-blue-700 hover:bg-blue-50 dark:border-blue-700 dark:bg-blue-900/20 dark:text-blue-400 dark:hover:bg-blue-900/30">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                </svg>
                                Edit
                            </button>
                            {{-- <form action="{{ route('kegiatan.destroy', $kegiatan->id_kegiatan) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="button"
    ...
    @push('scripts')
    <script>
        // Function untuk autocomplete pegawai
        function pegawaiAutocomplete() {
            return {
                ...
                get filteredPegawais() {
                    if (this.search.length === 0) return this.pegawais;
                ...
                selectPegawai(pegawai) {
                    this.search = pegawai.nama_pegawai;
                    this.selectedId = pegawai.id_pegawai;
                    this.open = false;
                    this.highlightedIndex = -1;
                },

                highlightNext() {
                    if (this.highlightedIndex < this.filteredPegawais.length - 1) this.highlightedIndex++;
                },
                ...
                selectHighlighted() {
                    if (this.highlightedIndex >= 0 && this.highlightedIndex < this.filteredPegawais.length) {
        ...
        function openEditModal(kegiatan) {
            // Setup data untuk modal
            const modalData = {
                mode: 'edit',
                actionUrl: `{{ url('kegiatan') }}/${kegiatan.id_kegiatan}`,
                formData: {
                ...
                ikiOptions: [],
                // Inisialisasi nama ketua untuk autocomplete
                pegawaiSearch: kegiatan.penanggung_jawab?.nama_pegawai || ''
            };

            // Dispatch event ke smart modal
            ...
            // Load IKI options jika ada RK JPT
            if (kegiatan.rk_jpt) {
                fetch(`/rencana-indikator-jpt/${kegiatan.rk_jpt}/indikator`)
                    .then(res => res.json())
                    .then(data => {
                        // Update ikiOptions di modal
                        const modal = document.querySelector('#modal-kegiatan-rk-ketua');
                        if (modal && modal.__x) {
                            modal.__x.$data.ikiOptions = data;
                        }
                    });
            }
        }

        // Function untuk load IKI options
        function loadIkiOptions() {
            const modal = document.querySelector('#modal-kegiatan-rk-ketua');
            if (!modal || !modal.__x) return;

            const alpineData = modal.__x.$data;
            const rkId = alpineData.formData.rk_jpt;

            if (!rkId) {
                alpineData.ikiOptions = [];
                return;
            }
            ...
            fetch(`/rencana-indikator-jpt/${rkId}/indikator`)
                .then(res => res.json())
                .then(data => {
                    alpineData.ikiOptions = data;
                    // Reset IKI selection jika IKI sebelumnya tidak termasuk dalam options baru
                    if (alpineData.formData.iki_jpt && !data.some(iki => iki.id == alpineData.formData.iki_jpt)) {
                        alpineData.formData.iki_jpt = '';
                    }
                });
        }

        // Inisialisasi Alpine.js data untuk modal
        document.addEventListener('alpine:init', () => {
            Alpine.data('modalKegiatan', () => ({
                mode: 'create',
                actionUrl: '',
                formData: {
                    id_penanggung_jawab: '',
                ...
                init() {
                    // Setup modal ketika dibuka
                    window.addEventListener('open-smart-modal', (e) => {
                        if (e.detail.modalId !== 'modal-kegiatan-rk-ketua') return;

                        const data = e.detail.data || {};
                        this.mode = data.mode || 'create';
                        this.actionUrl = data.actionUrl || '';
                        this.formData = data.formData || this.formData;
                        this.ikiOptions = data.ikiOptions || [];
                        this.pegawaiSearch = data.pegawaiSearch || '';

                        // Jika ada RK JPT tapi belum ada IKI options, load sekarang
                        if (this.formData.rk_jpt && this.ikiOptions.length === 0) {
                            loadIkiOptions();
                        }
                    });
                }
        ...
        // Setup modal content ketika DOM siap
        document.addEventListener('DOMContentLoaded', function() {
            const modalContainer = document.querySelector('#modal-kegiatan-rk-ketua');
            const modalTemplate = document.querySelector('#modal-kegiatan-template');

            if (modalContainer && modalTemplate) {
                // Clone template ke modal
                const modalContent = modalTemplate.content.cloneNode(true);
                modalContainer.appendChild(modalContent);

                // Init Alpine.js pada modal
