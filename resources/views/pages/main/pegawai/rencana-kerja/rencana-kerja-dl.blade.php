                                                                        ...
                                                                        <div class="flex flex-wrap items-center gap-1.5 min-h-[26px]">
                                                                            @if ($penugasan->status_dl === 'Ditolak')
                                                                                @if (Auth::user()->active_role === 'Ketua Tim' && Auth::user()->id_pegawai === $penugasan->subKegiatan->kegiatan->id_penanggung_jawab)
                                                                                    <button type="button" @click="$dispatch('open-smart-modal', { modalId: 'modal-verifikasi-dl', key: @js($penugasan->id_penugasan), data: { nama_pegawai: @js($penugasan->anggota->nama_pegawai), jenis_kegiatan: @js($penugasan->jenisKegiatan?->jenis_kegiatan ?? 'Isi Sendiri'), tanggal_mulai: @js($penugasan->tanggal_mulai->format('d M Y')), tanggal_selesai: @js($penugasan->tanggal_selesai->format('d M Y')), } })" class="px-2.5 py-1 text-[10px] font-bold bg-blue-600 text-white rounded hover:bg-blue-700 transition-colors shadow-sm w-full flex justify-center">Ajukan Kembali</button>
                                                                                @endif
                                                                                @if (Auth::user()->active_role === 'Pimpinan')
