...
class="modal fixed inset-0 z-99999 flex items-center justify-center overflow-hidden p-4"
...
{{ $attributes }}>
    ...
    <div class="relative flex items-center justify-center"
        ...
        <div @click.stop 
                ...
                <template x-if="!isLoading && announcements.length > 0">
                    <div class="flex h-full flex-col">
                        <div x-show="announcements[currentIndex].image" 
                            ...
                            <img :src="announcements[currentIndex].image" 
                                 :alt="announcements[currentIndex].title"
                                 class="h-full w-full object-contain">
                        </div>
                        
                        <div class="shrink-0 overflow-y-auto pr-1" 
                             :class="{ 'max-h-[20%]': announcements[currentIndex].image, 'h-full': !announcements[currentIndex].image }">
                            <div class="prose prose-sm max-w-none dark:prose-invert">
                                <p class="whitespace-pre-line text-sm text-gray-600 dark:text-gray-300" 
                                   x-text="announcements[currentIndex].content"></p>
                            </div>
                        </div>
                    </div>
                </template>
            ...
            <!-- Footer -->
            <div class="shrink-0 border-t border-gray-200 px-4 py-2 dark:border-gray-700" x-show="announcements.length > 0">
        ...
        <button @click="nextAnnouncement()"
            x-show="currentIndex < announcements.length - 1"
            x-transition:enter="transition-all duration-300 ease-out"
            x-transition:enter-start="opacity-0 translate-x-4"
            x-transition:enter-end="opacity-100 translate-x-0"
